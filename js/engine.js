...
export const DRIVE_MS_PER_EDGE = 2200;
...
export const MIN_AVAILABLE_VEHICLES_BEFORE_FAILURE = 7;
...
export function getTotalAvailableVehicleCount(){return vehicles.filter(vehicle=>vehicle.status===STATUS.AVAILABLE).length;}
...
export class Engine {
 ...
 startSimulationClock(now=this.simulationNow()){this.simulationClock.start(now);}
 ...
 createIncident(options={}) {
  ...
  if(["automatic","manualVehicle"].includes(sessionConfig.operationMode))this.startSimulationClock();
  ...
  const incident={id:`INC-${Date.now()}-${++this.sequence}`,type,district:district.id,districtId:district.id,x:position.x,y:position.y,markerVisible:true,status:"OPEN",createdAt:this.simulationNow(),prisonId:null,baseTravelTime:null,travelTime:null,capacityExceeded:false,requiredUnits,assignedVehicleIds:[],arrivedVehicleIds:[],transportVehicleId:null,vehicleId:null,sceneBusyDurationSeconds:type==="onscene"?getRandomOnSceneBusySeconds():null};
 ...
 dispatchTrainingSelection(incident,ids){
  ...
  this.clearVehicleSelection();simulator.activeIncident=null;this.activateTrainingIncidentIfUnambiguous();
 ...
 confirmManualDispatch(){
  ...
  const name=getDistrictById(incident.district)?.name;this.resetInputCycle();return this.result(true,`[DISPATCH] ${ids.join(", ")} handmatig ingezet voor melding ${name} (${incident.assignedVehicleIds.length}/${incident.requiredUnits}).`,{events});
 }
 dispatchVehicle(){
 ...
 startDispatch({incidentId,vehicleId,prisonId}){
  ...
  const now=this.simulationNow(),departureAt=this.scheduleMovementStart(id,now+DISPATCH_DEPARTURE_DELAY_MS),dispatch={id,vehicleId:vehicle.id,incidentId:incident.id,phase:STATUS.TO_INCIDENT,departureAt,originDistrictId:origin,returnTargetDistrictId,incidentDistrictId:incident.district,incidentX:incident.x,incidentY:incident.y,prisonDistrictId:prison?.id||null,prisonPosition,prisonX:parkingPosition?.x,prisonY:parkingPosition?.y,routeToIncident,routeToPrison,returnRoute:getShortestRoute(incident.district,returnTargetDistrictId),prisonReturnRoute:prison?getShortestRoute(prison.id,returnTargetDistrictId):[],phaseStartTime:departureAt,busySeconds:incident.type==="onscene"?incident.sceneBusyDurationSeconds:incident.travelTime,fromX:vehicle.x,fromY:vehicle.y,toX:incident.x,toY:incident.y};dispatch.pathPoints=buildMovementPath({fromX:vehicle.x,fromY:vehicle.y,routeDistrictIds:routeToIncident,toX:incident.x,toY:incident.y});this.activeDispatches.set(id,dispatch);simulator.activeRoutes.push({id:`${id}-to-incident`,type:"dispatch",pathPoints:dispatch.pathPoints});return this.result(true,`[DISPATCH] ${vehicle.id} aangestuurd; vertrek over circa 2 seconden (${incident.assignedVehicleIds.length}/${incident.requiredUnits}).`,{vehicle,district:getDistrictById(incident.district),events:[...capacityEvents,...this.evaluateHardAvailabilityFailure(now)]});
 }
 update(wallNow=performance.now()){
  // Pausing freezes operational time; rendering remains in the application loop.
  if(simulator.gameOver)return[];
  if(simulator.paused)return[];
  const now=this.simulationNow(wallNow);
  this.simulationClock.update(now);
  this.releaseMovementStarts(now);
  const events=[];
  events.push(...this.evaluateScheduledCoverage(now));
  ...
  events.push(...this.startNextQueuedReposition(now));
 ...
 updateDispatch(d,now){
  ...
  if(d.phase===STATUS.ON_SCENE){
   ...
   this.phase(d,STATUS.RETURNING,this.scheduleMovementStart(`${d.id}-return`,now),d.returnRoute,getDistrictById(d.returnTargetDistrictId));v.status=STATUS.RETURNING;delete v.onSceneArrivedAt;simulator.activeRoutes.push({id:`${d.id}-return`,type:"return",pathPoints:d.pathPoints});
  ...
  if(d.phase===STATUS.TO_INCIDENT){
   ...
   if(incident.type==="onscene"){
    ...
    if(incident.arrivedVehicleIds.length>=incident.requiredUnits)this.scheduleCoverageEvaluation(incident,now);
   ...
   this.scheduleCoverageEvaluation(incident,now);this.handleIncident(incident.id,now);simulator.incidentsHandled++;
 ...
 scheduleCoverageEvaluation(incident,now=performance.now()){if(!incident.coverageEvaluationAt&&!incident.coverageEvaluatedAt)incident.coverageEvaluationAt=now+getRepositionEvaluationDelayMs();}
 evaluateScheduledCoverage(now=performance.now()){
  const due=[...simulator.incidents,...simulator.incidentHistory].filter((incident,index,list)=>incident.coverageEvaluationAt&&incident.coverageEvaluationAt<=now&&!incident.coverageEvaluatedAt&&list.findIndex(item=>item.id===incident.id)===index);
 ...
 getDistrictsNeedingCoverage(){return this.getCriticalCoverageDistricts();}
 hasRelevantQueuedOrIncomingReposition(districtsWithNeed=this.getDistrictsNeedingCoverage()){return districtsWithNeed.some(d=>this.getIncomingRepositions(d.id)>0);}
 hasRelevantIncomingReposition(){return this.hasRelevantQueuedOrIncomingReposition();}
 canAnyDistrictDonateVehicle(context={}){return districts.some(d=>this.canDistrictDonateVehicle(d.id,context));}
 ...
 evaluateCoverageFailure(now=performance.now()){
  if(simulator.gameOver)return[];
  const hardFailure=this.evaluateHardAvailabilityFailure(now);if(hardFailure.length)return hardFailure;
  const evaluationPending=[...simulator.incidents,...simulator.incidentHistory].some(incident=>incident.coverageEvaluationAt&&!incident.coverageEvaluatedAt);
  const incidentTravelPending=[...this.activeDispatches.values()].some(dispatch=>[STATUS.TO_INCIDENT,"WAITING_AT_INCIDENT"].includes(dispatch.phase));
  if(evaluationPending||incidentTravelPending){this.coverageLossStartedAt.clear();return[];}
  const districtsWithNeed=this.getDistrictsNeedingCoverage();
  for(const district of districts)if(!districtsWithNeed.includes(district))this.coverageLossStartedAt.delete(district.id);
  if(!districtsWithNeed.length||this.canAnyDistrictDonateVehicle()||this.hasRelevantQueuedOrIncomingReposition(districtsWithNeed)){this.coverageLossStartedAt.clear();return[];}
  let critical=null;
  for(const district of districtsWithNeed){
  ...
  return[
   ...
   this.triggerRepositioningFailure(critical,now)
  ];
 }
 evaluateRepositionImpossible(now=performance.now()){return this.evaluateCoverageFailure(now);}
 ...
 getProjectedCoverage(districtId,plannedMoves=[]){const incoming=this.getIncomingRepositions(districtId)+plannedMoves.filter(move=>move.toDistrictId===districtId).length,outgoing=plannedMoves.filter(move=>move.fromDistrictId===districtId).length;return this.availableCount(districtId)+incoming-outgoing;}
 canDistrictDonateVehicle(districtId,context={}){const moves=context.plannedMoves||[],reserved=new Set(context.reservedVehicleIds||[]),available=vehicles.filter(v=>v.district===districtId&&v.status===STATUS.AVAILABLE&&!v.incident&&!reserved.has(v.id));if(available.length<2)return false;const minimum=(sessionConfig.hotzoneDistrictIds||[]).includes(districtId)?(context.targets||this.getCoverageTargets()).hotzoneMinimum:1;const candidate=this.rankVehiclesForReposition(available,districtId)[0];return !!candidate&&this.getProjectedCoverage(districtId,[...moves,{fromDistrictId:districtId,toDistrictId:"__probe__"}])>=minimum;}
 findDonorPath(targetDistrictId,options={}){
  const requiredOrigin=options.requiredOriginDistrictId,queue=[[targetDistrictId]],visited=new Set([targetDistrictId]),targets=this.getCoverageTargets();
 ...
 validateRepositionPlan(plan){
  ...
  const hotzonePriority=["minimum","parity"].includes(plan.coverageRule);
  if(!hotzonePriority&&!plan.moves.every(move=>projected[move.fromDistrictId]>=projected[move.toDistrictId]))return false;
  if(hotzonePriority&&(!hotzones.has(plan.targetDistrictId)||projected[plan.targetDistrictId]<=targets.effectiveByDistrict[plan.targetDistrictId]||districts.some(d=>projected[d.id]<(hotzones.has(d.id)&&d.id!==plan.targetDistrictId?targets.effectiveByDistrict[d.id]:1))))return false;
  // A Hotzone may donate only if it remains above both its minimum and every
  // projected non-Hotzone. This prevents ordinary balancing from undoing a
  // completed priority move.
  const projectedNonHotzoneMax=Math.max(0,...districts.filter(d=>!hotzones.has(d.id)).map(d=>projected[d.id]));
  if(plan.moves.some(move=>hotzones.has(move.fromDistrictId)&&projected[move.fromDistrictId]<Math.max(targets.hotzoneMinimum,projectedNonHotzoneMax)))return false;
  // Priority corrections intentionally may invert a 3/2 pair. Their safety
  // constraints above replace (rather than supplement) the symmetric
  // variance test used by ordinary balancing.
  if(hotzonePriority)return true;
 ...
 startRepositionPlan(plan){
  ...
  const events=[];for(const move of plan.moves){const vehicle=vehicles.find(v=>v.id===move.vehicleId),origin=getDistrictById(move.fromDistrictId),target=getDistrictById(move.toDistrictId),id=`REP-${++this.repositionSequence}`,route=[move.fromDistrictId,move.toDistrictId];Object.assign(move,{id});vehicle.status=STATUS.REPOSITION_QUEUED;this.repositionQueue.push({id,planId:plan.id,type:plan.repositionType,originDistrictId:origin.id,sourceDistrictId:origin.id,vehicleId:vehicle.id,targetDistrictId:target.id,route,fromX:vehicle.x,fromY:vehicle.y,toX:target.x,toY:target.y,cascade:plan.moves.length>1});events.push({type:"log",message:`[WACHTRIJ] ${vehicle.id}: ${origin.name} → ${target.name} ingepland.`});}this.activeRepositionPlans.set(plan.id,plan);events.push(...this.startNextQueuedReposition(),...this.evaluateHardAvailabilityFailure());return events;
 }
 startNextQueuedReposition(now=performance.now()){
 ...
 evaluateHomeReturns(now=performance.now()){
  ...
  const targets=this.getCoverageTargets(),hotzones=new Set(sessionConfig.hotzoneDistrictIds||[]);
  const nonHotzoneIds=districts.filter(d=>!hotzones.has(d.id)).map(d=>d.id);
  const vehicle=vehicles.filter(item=>item.status===STATUS.AVAILABLE&&!item.incident&&item.district!==item.homeDistrict&&now-(item.lastRepositionedAt??-Infinity)>=REPOSITION_COOLDOWN_MS&&!blockedDistricts.has(item.district)&&!blockedDistricts.has(item.homeDistrict)&&this.getEffectiveCoverage(item.district)-1>=this.getEffectiveCoverage(item.homeDistrict)+1&&(!hotzones.has(item.district)||this.getEffectiveCoverage(item.district)-1>=Math.max(targets.hotzoneMinimum,...nonHotzoneIds.map(id=>this.getEffectiveCoverage(id)+(id===item.homeDistrict?1:0))))).sort((a,b)=>Number(isSpecialVehicle(b))-Number(isSpecialVehicle(a)))[0];
  if(!vehicle)return[];
  ...
  const id=`REP-${++this.repositionSequence}`,reposition={id,type:"restore",originDistrictId:origin.id,sourceDistrictId:origin.id,vehicleId:vehicle.id,targetDistrictId:target.id,route,phaseStartTime:this.scheduleMovementStart(id,now),fromX:vehicle.x,fromY:vehicle.y,toX:target.x,toY:target.y};
 ...
 ensureCoverage(){
  ...
  // Priority 1: zero coverage always precedes Hotzone optimization.
  for(const target of this.getCriticalCoverageDistricts()){const donor=this.findCoverageDonor(target,targets,hotzones.has(target.id),hotzones.has(target.id)?"minimum":"normal");if(!donor)continue;const event=this.startAutomaticReposition(donor,target,hotzones.has(target.id)?"minimum":"normal");if(event)return event.planEvents||[event];}
  ...
  // Hard rule B: the least-covered Hotzone must match or exceed the richest
  // non-Hotzone; unlike ordinary balancing, a 3/2 pair may become 2/3.
  if(!deficient.length&&state.minHotzoneCoverage!==null&&state.maxNonHotzoneCoverage>state.minHotzoneCoverage){const target=[...state.hotzones].sort((a,b)=>a.effectiveCoverage-b.effectiveCoverage||a.district.id.localeCompare(b.district.id))[0]?.district;for(const donorEntry of [...state.nonHotzones].sort((a,b)=>b.effectiveCoverage-a.effectiveCoverage||a.district.id.localeCompare(b.district.id))){if(donorEntry.effectiveCoverage!==state.maxNonHotzoneCoverage||donorEntry.effectiveCoverage<=1)continue;const event=this.startAutomaticReposition(donorEntry.district,target,"parity");if(event)return event.planEvents||[event];}}
 ...
 startManualRepositionDispatch(){
  ...
  const origin=getDistrictById(vehicle.district),plan=this.buildRepositionPlan(target.id,{type:"manual",requiredVehicleId:vehicle.id});
  if(!plan){Object.assign(state,{phase:"selectDistrict",targetDistrictId:null});this.removeRoute("manual-reposition-preview");return this.result(false,"Geen veilige herpositioneringsketen beschikbaar.");}
  const events=this.startRepositionPlan(plan);this.removeRoute("manual-reposition-preview");this.clearManualReposition();this.activateTrainingIncidentIfUnambiguous();
  return this.result(true,`[HANDMATIGE HERPOSITIONERING] Dekking wordt doorgeschoven naar ${target.name}.`,{events:[{type:"log",message:`[HERPOSITIONERING] Dekking wordt doorgeschoven naar ${target.name}.`},...events,...this.ensureCoverage()]});
 }
 confirmManualReposition(){return this.result(false,"[FOUT] Kies een doeldistrict om de herpositionering direct te starten.");}
 scheduleNextIncident(now=this.simulationNow()){const delay=getRandomIncidentDelaySeconds();Object.assign(simulator.autoplayState,{nextDelaySeconds:delay,nextIncidentAt:now+delay*1000});return delay;}
 toggleAutoplay(){
  const training=sessionConfig.operationMode==="repositionTraining";if(!["autoplay","repositionTraining"].includes(sessionConfig.operationMode)||simulator.gameOver)return this.result(false,"[FOUT] Automatische meldingen zijn niet actief.");
  const state=simulator.autoplayState,wallNow=performance.now();state.running=!state.running;
  if(state.running){if(simulator.paused&&this.pauseStartedAt!==null)this.totalPausedMs+=Math.max(0,wallNow-this.pauseStartedAt);simulator.paused=false;this.pauseStartedAt=null;this.startSimulationClock();if(state.nextIncidentAt===null)this.scheduleNextIncident();}
  else{simulator.paused=true;this.pauseStartedAt=wallNow;this.simulationClock.pause(this.simulationNow(wallNow));}
