...
export class Engine {
 ...
 startManualReposition(){
  ...
  if(this.step!==STEPS.INCIDENT||simulator.vehicleSelection.active)return this.result(false,"[FOUT] Maak eerst de huidige melding af.");
  Object.assign(simulator.manualRepositionState,{active:true,selectedVehicleId:null,targetDistrictId:null});
 ...
 clearManualReposition(){Object.assign(simulator.manualRepositionState,{active:false,selectedVehicleId:null,targetDistrictId:null});this.removeRoute("manual-reposition-preview");}
 selectRepositionVehicle(vehicleId){
  ...
  Object.assign(state,{selectedVehicleId:vehicle.id,targetDistrictId:null});this.removeRoute("manual-reposition-preview");
 ...
 confirmManualReposition(){
  const state=simulator.manualRepositionState,vehicle=vehicles.find(v=>v.id===state.selectedVehicleId),target=getDistrictById(state.targetDistrictId);
  if(!state.active||!vehicle||!target)return this.result(false,"[FOUT] Kies eerst een voertuig en doeldistrict.");
  if(vehicle.status!==STATUS.AVAILABLE||vehicle.incident||[...this.activeRepositions.values()].some(r=>r.vehicleId===vehicle.id)){this.clearManualReposition();return this.result(false,`[FOUT] ${vehicle.id} is ondertussen niet meer beschikbaar. Selectie is gewist.`);}
  const route=getShortestRoute(vehicle.district,target.id);if(!route.length||target.id===vehicle.district)return this.result(false,"[FOUT] Herpositioneringsroute is ongeldig.");
  ...
  return this.result(true,`[HANDMATIGE HERPOSITIONERING] ${vehicle.id} onderweg van ${origin.name} naar ${target.name}.`,{events:[{type:"manualRepositionStarted",vehicle,district:target,origin},...this.ensureCoverage()]});
 }
