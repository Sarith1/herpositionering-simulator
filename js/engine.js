...
export class Engine {
 constructor(){ this.simulationClock=new SimulationClock(); this.activeDispatches=new Map(); this.activeRepositions=new Map(); this.repositionQueue=[]; this.movementStartQueue=[]; this.lastMovementStartedAt=-Infinity; this.activeRepositionPlans=new Map(); this.coverageLossStartedAt=new Map(); this.sequence=0; this.repositionSequence=0; this.repositionPlanSequence=0; this.manualRepositionStarting=false; }
 scheduleMovementStart(movementId,earliestAt=performance.now()){const startAt=Math.max(earliestAt,this.lastMovementStartedAt+MOVEMENT_START_GAP_MS);this.lastMovementStartedAt=startAt;this.movementStartQueue.push({movementId,startAt});return startAt;}
 releaseMovementStarts(now){this.movementStartQueue=this.movementStartQueue.filter(item=>item.startAt>now);}
 startSimulationClock(now=performance.now()){this.simulationClock.start(now);}
 ...
 toggleAutoplay(){const training=sessionConfig.operationMode==="repositionTraining";if(!["autoplay","repositionTraining"].includes(sessionConfig.operationMode)||simulator.gameOver)return this.result(false,"[FOUT] Automatische meldingen zijn niet actief.");const state=simulator.autoplayState;state.running=!state.running;if(state.running){this.startSimulationClock();this.scheduleNextIncident();}else Object.assign(state,{nextIncidentAt:null,nextDelaySeconds:null});return this.result(true,training?`[MODUS] Herpositioneringsmodus ${state.running?"gestart":"gepauzeerd"}. ${state.running?"Beheer de dekking met H.":"Lopende opdrachten rijden door."}`:`[MODUS] Autoplay ${state.running?"gestart":"gepauzeerd"}. Lopende opdrachten rijden door.`);}
