...
export class Engine {
 constructor(){ this.simulationClock=new SimulationClock(); this.activeDispatches=new Map(); this.activeRepositions=new Map(); this.repositionQueue=[]; this.activeRepositionPlans=new Map(); this.coverageLossStartedAt=new Map(); this.sequence=0; this.repositionSequence=0; this.repositionPlanSequence=0; this.manualRepositionStarting=false; }
 ...
 canDistrictDonateVehicle(districtId,context={}){const moves=context.plannedMoves||[],reserved=new Set(context.reservedVehicleIds||[]),minimum=(sessionConfig.hotzoneDistrictIds||[]).includes(districtId)?(context.targets||this.getCoverageTargets()).hotzoneMinimum:1;const candidate=this.rankVehiclesForReposition(vehicles.filter(v=>v.district===districtId&&v.status===STATUS.AVAILABLE&&!v.incident&&!reserved.has(v.id)),districtId)[0];return !!candidate&&this.getProjectedCoverage(districtId,[...moves,{fromDistrictId:districtId,toDistrictId:"__probe__"}])>=minimum;}
