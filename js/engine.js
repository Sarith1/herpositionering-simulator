...
export class Engine {
 ...
 enterVehicleSelection(){
  ...
  return this.result(true,`[SELECTIE] Kies een voertuig voor melding ${getDistrictById(incident.district)?.name}.`);
 }
 selectIncident(incidentId){
  ...
  const incident=simulator.incidents.find(i=>i.id===incidentId&&(i.status==="OPEN"||i.status==="WAITING"));if(!incident)return this.result(false,"[FOUT] Deze melding is niet meer open.");
 ...
 selectVehicle(vehicleId){
  ...
  if(sessionConfig.operationMode!=="manualVehicle"||!selection.active||simulator.gameOver)return this.result(false,"[FOUT] Start eerst de voertuigkeuze met ‘Pak melding op’. ");
  const vehicle=vehicles.find(v=>v.id===vehicleId),incident=simulator.incidents.find(i=>i.id===selection.incidentId&&(i.status==="OPEN"||i.status==="WAITING"));
 ...
 dispatchVehicle(){
  ...
  if(sessionConfig.operationMode==="manualVehicle"){
   if(!simulator.vehicleSelection.active)return this.enterVehicleSelection();
   const selection=simulator.vehicleSelection;if(!selection.selectedVehicleId)return this.result(false,"[FOUT] Selecteer eerst een beschikbaar voertuig op de kaart.");if(selection.confirming)return this.result(false,"[FOUT] Deze inzet wordt al verwerkt.");selection.confirming=true;
   const incident=simulator.incidents.find(i=>i.id===selection.incidentId&&(i.status==="OPEN"||i.status==="WAITING")),vehicle=vehicles.find(v=>v.id===selection.selectedVehicleId);
   if(!incident||!vehicle||vehicle.status!==STATUS.AVAILABLE||vehicle.incident){selection.confirming=false;return this.result(false,"[FOUT] Voertuig of melding is niet meer beschikbaar.");}
   const vehicleId=vehicle.id,districtName=getDistrictById(incident.district)?.name,result=this.assignIncident(incident,vehicleId);if(result.success){this.clearVehicleSelection();result.message=`[DISPATCH] ${vehicleId} handmatig ingezet voor melding ${districtName}.`;}else selection.confirming=false;return result;
  }
  const incident=this.oldestOpen();if(!incident)return this.result(false,"[FOUT] Er is geen open melding.");return this.assignIncident(incident);
 }
