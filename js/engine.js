...
export class Engine {
 ...
 selectPrison(){ if(this.step!==STEPS.PRISON)return this.result(false,"[FOUT] Plaats eerst een melding.");const i=simulator.activeIncident;if(!i)return this.result(false,"[FOUT] Plaats eerst een melding.");this.prepareIncident(i);Object.assign(simulator.inputCycleState,{prisonId:i.prisonId,travelTime:i.travelTime});this.setStep(STEPS.TRAVEL_TIME);return this.result(true,`[CEL] ${getDistrictById(i.prisonId)?.name} geselecteerd.`); }
 calculateTravelTime(){ if(this.step!==STEPS.TRAVEL_TIME)return this.result(false,"[FOUT] Selecteer eerst een cel.");const i=simulator.activeIncident;if(!i?.prisonId)return this.result(false,"[FOUT] Selecteer eerst een cel.");this.setStep(STEPS.DISPATCH);return this.result(true,`[REISTIJD] Geschatte reistijd: ${i.travelTime} seconden.`); }
 ...
 startDispatch({incidentId,vehicleId,prisonId}){
  ...
  incident.status="ASSIGNED";incident.vehicleId=vehicle.id;this.activeDispatches.set(id,dispatch);simulator.activeRoutes.push({id,route:routeToIncident,type:"dispatch",destination:{x:incident.x,y:incident.y}});this.resetInputCycle();
 ...
 startManualReposition(){
  ...
  Object.assign(simulator.manualRepositionState,{active:true,selectedVehicleId:null,targetDistrictId:null,confirming:false});
 ...
 clearManualReposition(){Object.assign(simulator.manualRepositionState,{active:false,selectedVehicleId:null,targetDistrictId:null,confirming:false});this.removeRoute("manual-reposition-preview");}
 selectRepositionVehicle(vehicleId){
  const state=simulator.manualRepositionState,vehicle=vehicles.find(v=>v.id===vehicleId);
  if(!state.active)return this.result(false,"[FOUT] Start eerst ‘Herpositioneer voertuig’.");
  ...
  Object.assign(state,{selectedVehicleId:vehicle.id,targetDistrictId:null,confirming:false});this.removeRoute("manual-reposition-preview");
 ...
 selectRepositionTarget(targetDistrictId){
  ...
  if(!state.active||!vehicle)return this.result(false,"[FOUT] Kies eerst een beschikbaar voertuig.");
  ...
  state.targetDistrictId=target.id;this.removeRoute("manual-reposition-preview");simulator.activeRoutes.push({id:"manual-reposition-preview",route,type:"selection-preview"});
 ...
 confirmManualReposition(){
  ...
  if(!state.active||simulator.gameOver)return this.result(false,"[FOUT] Herpositioneren is niet meer mogelijk.");
  if(state.confirming)return this.result(false,"[FOUT] Deze herpositionering wordt al verwerkt.");
  ...
  if(unavailable){const vehicleId=vehicle?.id||state.selectedVehicleId;Object.assign(state,{selectedVehicleId:null,targetDistrictId:null,confirming:false});this.removeRoute("manual-reposition-preview");return this.result(false,`[WAARSCHUWING] ${vehicleId} is niet meer beschikbaar. Kies opnieuw een voertuig.`);}
  if(!target){Object.assign(state,{targetDistrictId:null,confirming:false});this.removeRoute("manual-reposition-preview");return this.result(false,"[WAARSCHUWING] Het doeldistrict is niet meer geldig. Kies opnieuw een doeldistrict.");}
  const route=getShortestRoute(vehicle.district,target.id);if(!route.length||target.id===vehicle.district){Object.assign(state,{targetDistrictId:null,confirming:false});this.removeRoute("manual-reposition-preview");return this.result(false,"[WAARSCHUWING] Herpositioneringsroute is niet meer geldig. Kies opnieuw een doeldistrict.");}
