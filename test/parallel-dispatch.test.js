...
test("five simultaneous dispatches retain unique phase routes and input-cycle isolation", () => {
  const engine = new Engine(); engine.reset({ operationMode: "automatic", vehiclesPerDistrict: Object.fromEntries(districts.map(d => [d.id, 5])) });
  const ids=[];
  ...
  assert.equal(ids.slice(1).every(id=>simulator.incidents.find(item=>item.id===id)?.status==="ASSIGNED"),true);
});
